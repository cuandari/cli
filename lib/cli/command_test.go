...
func TestNewCommandDefaults(t *testing.T) {
	...
	a.True(*c.AllowImplicitCommands)
...
func TestSyscallDeniedActionSet(t *testing.T) {
	...
	tests := []struct {
		input   string
		want    cli.SyscallDeniedAction
		wantErr bool
	}{
		{"kill", cli.KillAction, false},
		{"error", cli.ErrorAction, false},
		{"invalid", "", true},
	}
	...
	for _, tt := range tests {
		err := s.Set(tt.input)
		if tt.wantErr {
			a.Error(err, "expected error for %s", tt.input)
		} else {
			a.NoError(err)
			a.Equal(tt.want, s)
		}
...
func TestDefaultsRemainFalseUntilParsed(t *testing.T) {
	...
	a.False(*c.AllowNetworkLocalSockets)
